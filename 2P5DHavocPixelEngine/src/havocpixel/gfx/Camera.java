...
public class Camera {
	...
	public Camera(Handler game, float xOffset, float yOffset,double scale) {
		this.handler = game;
	...
	public void centerOnEntity0(Entity e) {
		xOffset = (e.$x()-handler.$width()/2+e.$width()/2);
		yOffset = (e.$y()-handler.$height()/2+e.$height()/2);
		//checkBlankSpace();
	}
	public void centerOnEntity(Entity e) {
		float cx=(e.$x()-handler.$width()/2+e.$width()/2);
		float cy=(e.$y()-handler.$height()/2+e.$height()/2);
		int v=100;//,l=0;
