...
public class Assets {
	...
	public static BufferedImage testLevel,title,pak;
	...
	//hooded skeleton
		public static BufferedImage[] hsUp,hsLeft,hsDown,hsRight,hAsUp,hAsLeft,hAsDown,hAsRight,hsIdle;
		public static BufferedImage[] hsiU,hsiL,hsiD,hsiR,hemerge;
		public static BufferedImage hshU,hshL,hshD,hshR;
	//basic skeleton
	...
	//human grunt
	public static BufferedImage[] hUp,hLeft,hDown,hRight,AhUp,AhLeft,AhDown,AhRight,AhUpAlt,AhLeftAlt,AhDownAlt,AhRightAlt,hIdle;
	...
	public static BufferedImage[] phUp,phLeft,phDown,phRight,pAhUp,pAhLeft,pAhDown,pAhRight,pAhUpAlt,pAhLeftAlt,pAhDownAlt,pAhRightAlt,phIdle;
	...
	//gelatinous cube
	public static BufferedImage[] ci,cd,cm,cs;
	...
	public static BufferedImage ku,kl,kd,kr,eku,ekl,ekd,ekr,pku,pkl,pkd,pkr;
	...
	//grenade
	public static BufferedImage gren,fireball,fireball0,wand0,wand1,wand2,icebolt,hpot0;
	//inv
	public static BufferedImage inventory,fog0,control;
	//smoke
	public static BufferedImage[] smoke,poof,energyBall;
	//gibs
	public static BufferedImage gib0,gib1,gib2,gib3,gibah,gibas,dGib0,dGib1,dGib2,dGibarm,dGibleg,dGibrib;
	//ghost
	public static BufferedImage gu,gl,gd,gr,gAu,gAl,gAd,gAr,ghu,ghl,ghr,
	                            gu2,gl2,gd2,gr2,gAu2,gAl2,gAd2,gAr2,ghu2,ghl2,ghr2,
	                            gu01,gl01,gd01,gr01,gAu01,gAl01,gAd01,gAr01,ghu01,ghl01,ghr01,
	                            gu012,gl012,gd012,gr012,gAu012,gAl012,gAd012,gAr012,ghu012,ghl012,ghr012,
	                            gu3,gl3,gd3,gr3,gAu3,gAl3,gAd3,gAr3,ghu3,ghl3,ghr3,rage;
	//incubus
	...
	//ground traps
	public static BufferedImage[] bt0,spike;
	public static BufferedImage gbt0,gbt1,gbt2,gbt3,aspike;
	//fire
	public static BufferedImage[] fire,gore,gore0,fire0;
	public static BufferedImage[] venom;
	//windslash
	public static BufferedImage w0,w1,w2,w3,w02,w03;
	//corpses
	public static BufferedImage ct0,ct1,ct2,ct3,ct,skullpile;
	public static BufferedImage[] corpse,tcorpse,pole,blood,soot;
	//coins
	...
	//hurt screen
	//public static BufferedImage[] hurt50, hurt25;
	...
	public static void load() {
		...
		Textures controls=new Textures(ImageLoader.loadImage("/txr/controls.png"));
		Textures txr0=new Textures(ImageLoader.loadImage("/txr/txr0.png"));
		...
		Textures hskel=new Textures(ImageLoader.loadImage("/txr/hood_skel.png"));
		...
		Textures ghost3=new Textures(ImageLoader.loadImage("/txr/ghost3.png"));
		...
		Textures puff=new Textures(ImageLoader.loadImage("/txr/vanish.png"));
		Textures energy=new Textures(ImageLoader.loadImage("/txr/energy_alt.png"));
		Textures cube=new Textures(ImageLoader.loadImage("/txr/gel.png"));
		Textures sp=new Textures(ImageLoader.loadImage("/txr/spike.png"));
		Textures ven=new Textures(ImageLoader.loadImage("/txr/poison.png"));
		Textures coi=new Textures(ImageLoader.loadImage("/txr/coins.png"));
		Textures b=new Textures(ImageLoader.loadImage("/txr/blood.png"));
		Textures title0=new Textures(ImageLoader.loadImage("/txr/demotitle.png"));
		Textures pak0=new Textures(ImageLoader.loadImage("/txr/pressanykey.png"));
		Textures ouch0=new Textures(ImageLoader.loadImage("/txr/hurt_blood0.png"));
		Textures zwei0=new Textures(ImageLoader.loadImage("/txr/blood_zwei_alt.png"));
		
		System.out.print("["+Timer.time()+"] Loading Animations;\n");
		pak=pak0.crop(0,0,140,13);
		title=title0.crop(0,0,750,460);
		...
		/*
		hurt50=new BufferedImage[32];
		for(int i=0;i<32;i++){
			hurt50[i]=hurt02.crop(0,0,750,460);
			hurt50[i]=Utils.setAlpha(hurt50[i], (byte)(i*8));
			//System.out.print("|");
			System.out.println(hurt50[i].getType()==BufferedImage.TYPE_4BYTE_ABGR);
		}
		System.out.println("|");
		hurt25=new BufferedImage[32];
		for(int i=0;i<32;i++){
			hurt25[i]=hurt0.crop(0,0,750,460);
			hurt25[i]=Utils.setAlpha(hurt25[i], (byte)(i*8));
			System.out.print("|");
			//System.out.println(hurt25[i]);
		}
		System.out.println("|");
		*/
		
		//zweihander
		//zU,zL,zD,zR,zAU,zAL,zAD,zAR;
		//ziU,ziL,ziD,ziR,zDeath,zSpawn;
		...
		//ouch
		ouch=new BufferedImage[4][4];
		for(int i=0;i<4;i++)
			ouch[0][i]=ouch0.crop(px*i, px*0, px, px);
		for(int i=0;i<4;i++)
			ouch[1][i]=ouch0.crop(px*i, px*1, px, px);
		for(int i=0;i<4;i++)
			ouch[2][i]=ouch0.crop(px*(i+4), px*0, px, px);
		for(int i=0;i<4;i++)
			ouch[3][i]=ouch0.crop(px*(i+4), px*1, px, px);
		//blood
		...
		fire0=new BufferedImage[8];
		for(int i=0;i<8;i++)
			fire0[i]=fi.crop(px*i, px*1, px, px);
		venom=new BufferedImage[8];
		for(int i=0;i<8;i++)
			venom[i]=ven.crop(px*i, px*0, px, px);
		poof=new BufferedImage[8];
		for(int i=0;i<8;i++)
			poof[i]=puff.crop(px*i, px*0, px, px);
		energyBall=new BufferedImage[8];
		for(int i=0;i<8;i++){
			energyBall[i]=energy.crop(px*i, px*0, px, px);
			//energyBall[i]=Utils.invertImage(energyBall[i]);
		}
		
		//knife variants
		pkr=ven.crop(px*0, px*1, px, px);
		pkd=ven.crop(px*1, px*1, px, px);
		pkl=ven.crop(px*2, px*1, px, px);
		pku=ven.crop(px*3, px*1, px, px);
		ekr=ven.crop(px*4, px*1, px, px);
		ekd=ven.crop(px*5, px*1, px, px);
		ekl=ven.crop(px*6, px*1, px, px);
		eku=ven.crop(px*7, px*1, px, px);
		
		//gelatinous cube
		ci=new BufferedImage[2];
		cm=new BufferedImage[16];
		cs=new BufferedImage[8];
		cd=new BufferedImage[5];
		for(int i=0;i<8;i++)
			cm[i]=cube.crop(px*i, px*0, px, px);
		for(int i=0;i<8;i++)
			cm[i+8]=cube.crop(px*i, px*1, px, px);
		for(int i=0;i<8;i++)
			cs[i]=cube.crop(px*i, px*3, px, px);
		for(int i=0;i<5;i++)
			cd[i]=cube.crop(px*i, px*2, px, px);
		slime=cube.crop(px*5, px*2, px, px);
		ci[0]=cube.crop(px*6, px*2, px, px);
		ci[1]=cube.crop(px*7, px*2, px, px);
		...
		hpot0=tomb.crop(px*6, px*14, px, px);
		icebolt=tomb.crop(px*7, px*14, px, px);
		mb=tomb.crop(px*4, px*15, px, px);
		wand0=tomb.crop(px*5, px*15, px, px);
		wand1=tomb.crop(px*6, px*15, px, px);
		wand2=tomb.crop(px*7, px*15, px, px);
		...
		//ghost3
				gu3=ghost3.crop(px*2, px*0, px, px);
				gl3=ghost3.crop(px*1, px*0, px, px);
				gd3=ghost3.crop(px*0, px*0, px, px);
				gr3=ghost3.crop(px*3, px*0, px, px);
				gAu3=ghost3.crop(px*6, px*0, px, px);
				gAl3=ghost3.crop(px*5, px*0, px, px);
				gAd3=ghost3.crop(px*4, px*0, px, px);
				gAr3=ghost3.crop(px*7, px*0, px, px);
				ghu3=ghost3.crop(px*0, px*1, px, px);
				ghl3=ghost3.crop(px*1, px*1, px, px);
				ghr3=ghost3.crop(px*3, px*1, px, px);
		...
		//corpses
		corpse=new BufferedImage[10];
		tcorpse=new BufferedImage[5];
		corpse[0]=bun.crop(px*5, px*13, px, px);
		corpse[1]=bun.crop(px*5, px*12, px, px);
		corpse[2]=bun.crop(px*4, px*12, px, px);
		corpse[3]=bun.crop(px*4, px*13, px, px);
		for(int i=0;i<6;i++)
			corpse[i+4]=bun.crop(px*i, px*4, px, px);
		//trapped corpses
		for(int i=0;i<5;i++)
			tcorpse[i]=bun.crop(px*i, px*5, px, px);
		//poles
		pole=new BufferedImage[6];
		for(int i=0;i<6;i++)
			pole[i]=bun.crop(px*i, px*15, px, px*2);
		//corpse trees
		ct=bun.crop(px*6, px*12, px*2, px*3);
		ct0=bun.crop(px*2, px*8, px*2, px*3);
		ct1=bun.crop(px*2, px*11, px*2, px*3);
		ct2=bun.crop(px*0, px*8, px*2, px*3);
		ct3=bun.crop(px*0, px*11, px*2, px*3);
		//skullpile
		skullpile=bun.crop(px*6, px*15, px*2, px*2);
		
		//spike trap
		spike=new BufferedImage[4];
		for(int i=0;i<4;i++)
			spike[i]=sp.crop(px*i, px*0, px, px);
		aspike=sp.crop(px*4, px*0, px, px);
		...
		AhUpAlt=new BufferedImage[4];
		AhDownAlt=new BufferedImage[4];
		AhRightAlt=new BufferedImage[4];
		AhLeftAlt=new BufferedImage[4];
		...
		for(int i=0;i<4;i++)
			AhUpAlt[i]=human0.crop(px*i, px*(5+11), px, px*2);
		for(int i=0;i<4;i++)
			AhLeftAlt[i]=human0.crop(px*i*2, px*(7+11), px*2, px);
		for(int i=0;i<4;i++)
			AhDownAlt[i]=human0.crop(px*i, px*(8+11), px, px*2);
		for(int i=0;i<4;i++)
			AhRightAlt[i]=human0.crop(px*i*2, px*(10+11), px*2, px);
		for(int i=0;i<5;i++)
		...
		//hooded skeleton
				...
				for(int i=0;i<8;i++)
					hsIdle[i]=hskel.crop(px*i, px*4, px, px);
				for(int i=0;i<4;i++)
					hAsUp[i]=hskel.crop(px*i, px*5, px, px*2);
				for(int i=0;i<4;i++)
					hAsLeft[i]=hskel.crop(px*i*2, px*7, px*2, px);
				for(int i=0;i<4;i++)
					hAsDown[i]=hskel.crop(px*i, px*8, px, px*2);
				for(int i=0;i<4;i++)
					hAsRight[i]=hskel.crop(px*i*2, px*10, px*2, px);
				hshD=hskel.crop(px*0, px*11, px, px);
				hshL=hskel.crop(px*1, px*11, px, px);
				hshU=hskel.crop(px*2, px*11, px, px);
				hshR=hskel.crop(px*3, px*11, px, px);
				hsiD[0]=hskel.crop(px*4, px*11, px, px);
				hsiL[0]=hskel.crop(px*5, px*11, px, px);
				hsiU[0]=hskel.crop(px*6, px*11, px, px);
				hsiR[0]=hskel.crop(px*7, px*11, px, px);
				...
				for(int i=0;i<8;i++)
					hemerge[i]=hskel.crop(px*i, px*14, px, px);
				for(int i=8;i<16;i++)
					hemerge[i]=hskel.crop(px*(i-8), px*15, px, px);
				hemerge[15]=hskel.crop(px*6, px*15, px, px);
