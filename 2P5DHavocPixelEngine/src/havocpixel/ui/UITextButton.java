...
public class UITextButton extends UIObject{
	...
	public void render(Graphics g) {
		if(!boundsSet){
			this.bounds=new Rectangle((int)((x-g.getFontMetrics().stringWidth(label))*hdlr.$game().$currentScaleWidth()),
					(int)((y-g.getFontMetrics().getHeight())*hdlr.$game().$currentScaleHeight()),
					(int)(g.getFontMetrics().stringWidth(label)*2*hdlr.$game().$currentScaleWidth()),
					(int)(g.getFontMetrics().getHeight()*hdlr.$game().$currentScaleHeight()));
			boundsSet=true;
		}
